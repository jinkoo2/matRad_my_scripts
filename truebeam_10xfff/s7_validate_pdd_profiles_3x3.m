%% s9_validate_pdd_profiles_3x3.m  –  TrueBeam 10XFFF (10MV FFF)
% Validate TrueBeam 10XFFF machine in a homogeneous water phantom.
% Setup  : AP beam (gantry 0), 3x3 cm² open field, SSD = 100 cm.
% Outputs: (1) calculated PDD vs GBD 3x3 cm² PDD
%          (2) calculated crossline profiles at 2.4, 5, 10, 20, 30 cm
%              vs GBD 3x3 cm² profiles
%
% NOTE: 10FFF GBD provides the shallowest profile at 2.4 cm (not 1.5 cm).
%
% Phantom sized for 3x3 cm² to minimize runtime:
%   x/z : ±80 mm (covers GBD profile range of ±69 mm at 30 cm depth)
%   y   : 0–318 mm (covers GBD PDD range of 0–30 cm)
%   Voxel count: 80×160×80 = 1.0 M
%   Ray count  : ~256 bixels (16x16 at 2mm spacing)
%
% addMargin=false: prevents matRad from auto-dilating the target by
%   bixelWidth (2mm), which would shift rays from ±15mm to ±17mm
...
pln_tmp.machine       = 'TrueBeam_10XFFF';
...
Nx = 80;    % cols   x: -80 to +78 mm  (inline,     ±80 mm)
Ny = 160;   % rows   y:   0 to +318 mm (depth, beam direction)
Nz = 80;    % slices z: -80 to +78 mm  (crossline,  ±80 mm)

ct.x = (-(Nx/2) : (Nx/2 - 1)) * res;   % -80, -78, ..., 78 mm
ct.y = (0       : Ny - 1)      * res;   %   0,   2, ..., 318 mm
ct.z = (-(Nz/2) : (Nz/2 - 1)) * res;   % -80, -78, ..., 78 mm
...
% Target: exactly the 3x3 cm² field footprint (±15 mm).
ix_tgt = find(ct.x >= -15 & ct.x <= 15);
iz_tgt = find(ct.z >= -15 & ct.z <= 15);
...
cst{2,1} = 1;  cst{2,2} = 'PTV_3x3';  cst{2,3} = 'TARGET';
cst{2,4} = {V_target}; cst{2,5} = meta_ptv;  cst{2,6} = {};

%% 4. Plan: single AP beam, 3x3 cm², gantry 0°
...
pln.propStf.bixelWidth   = 2;       % 2 mm pencil-beam width.
                                    % NOTE: do NOT use 2.5 mm — it causes a size mismatch
                                    % in the SVD engine because intConvResolution = 0.5 mm:
                                    %   fieldLimit = ceil(bw/(2*intConvRes)) uses ceil
                                    %   Fpre       = ones(floor(bw/intConvRes)) uses floor
                                    % These agree only when bw is an even multiple of 0.5 mm.
                                    % 2.5 mm is an odd multiple (5×0.5): 6×6 vs 5×5 → crash.
                                    % 2 mm is an even multiple (4×0.5): 4×4 vs 4×4 → ok.
pln.propStf.isoCenter    = [0, 0, 0];
pln.propStf.addMargin    = false;   % keep rays at ±15mm; auto-margin would
                                    % expand to ±17mm (~3.4x3.4 cm² field)
...
profile_depths_cm = [2.4, 5, 10, 20, 30];   % 2.4 cm = shallowest GBD profile for 10FFF
...
gbdDir = '/gpfs/projects/KimGroup/projects/tps/matRad/my_scripts/TrueBeamGBD/10FFF Beam Data';
...
[depth_meas_cm, pdd_meas_pct] = readGBDColumn( ...
    fullfile(gbdDir, 'Open Field Depth Dose.csv'), 6, 7, '3x3');
...
profile_files = { ...
    'Open Field Profiles at 2.4cm.csv', ...
...
for k = 1:length(profile_depths_cm)
    [xm, dm] = readGBDColumn(fullfile(gbdDir, profile_files{k}), 8, 9, '3x3');
...
figure('Name','PDD – 3x3 cm², TrueBeam 10XFFF','Position',[50 100 700 500]);
...
title('PDD – TrueBeam 10XFFF, 3×3 cm², SSD=100 cm');
...
for k = 1:length(profile_depths_cm)
    figure('Name', sprintf('Profile %.1fcm – 3x3 cm²', profile_depths_cm(k)), ...
    ...
    xline([-1.5 1.5], 'k:', 'LineWidth',0.8);    % field edge markers
    xlabel('Off-axis position [cm]');
    ylabel('Relative Dose [%]');
    title(sprintf('Profile at %.1f cm (grid: %.1f mm) – TrueBeam 10XFFF, 3×3 cm²', ...
...
for k = 1:length(profile_depths_cm)
    ...
    for x_off = x_check
        for sgn = [1, -1]
            ...
            fprintf('    x = %+.1f cm: calc=%5.1f%%  meas=%5.1f%%  diff=%+.1f%%\n', ...
                xq, pc, pm, pc-pm);
        end
    end
...
%% 15. Save
save('s7_water_phantom_3x3_10xfff_results.mat', ...
    ...
    'profile_depths_cm');
fprintf('\nSaved: s7_water_phantom_3x3_10xfff_results.mat\n');
