%% s8_validate_pdd_profiles_10x10.m  –  TrueBeam 10XFFF (10MV FFF)
...
%
% Phantom sized for 10x10 cm² to minimize runtime:
%   x/z : ±130 mm (covers GBD profile range of ±114 mm at 30 cm depth)
%   y   : 0–318 mm (covers GBD PDD range of 0–30 cm)
%   Voxel count: 130×160×130 = 2.7 M
%   Ray count  : ~484 bixels (22x22 at 5mm spacing)
...
pln.propStf.addMargin    = false;   % keep rays at ±50mm; auto-margin would
                                    % expand to ±55mm (~11x11 cm² field)
...
figure('Name','PDD – 10x10 cm², TrueBeam 10XFFF','Position',[50 100 700 500]);
...
title('PDD – TrueBeam 10XFFF, 10×10 cm², SSD=100 cm');
...
for k = 1:length(profile_depths_cm)
    figure('Name', sprintf('Profile %.1fcm – 10x10 cm²', profile_depths_cm(k)), ...
    ...
    ylabel('Relative Dose [%]');
    title(sprintf('Profile at %.1f cm (grid: %.1f mm) – TrueBeam 10XFFF, 10×10 cm²', ...
...
%% 15. Save
save('s8_water_phantom_10x10_10xfff_results.mat', ...
    ...
    'profile_depths_cm');
fprintf('\nSaved: s8_water_phantom_10x10_10xfff_results.mat\n');
