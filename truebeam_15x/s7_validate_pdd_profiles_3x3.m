%% s9_validate_pdd_profiles_3x3.m  –  TrueBeam 15X (15MV flat)
% Validate TrueBeam 15X machine in a homogeneous water phantom.
% Setup  : AP beam (gantry 0), 3x3 cm² open field, SSD = 100 cm.
% Outputs: (1) calculated PDD vs GBD 3x3 cm² PDD
%          (2) calculated crossline profiles at 3, 5, 10, 20, 30 cm
...
profile_depths_cm = [3, 5, 10, 20, 30];   % 3 cm = shallowest GBD profile for 15MV
...
for k = 1:length(profile_depths_cm)
    ...
    fprintf('Profile at %.0f cm: nearest grid depth = %.1f mm (row %d)\n', ...
        profile_depths_cm(k), depth_used_mm(k), iy_d);
end

%% 11. Load GBD reference data
gbdDir = '/gpfs/projects/KimGroup/projects/tps/matRad/my_scripts/TrueBeamGBD/15MV Beam Data';
...
profile_files = { ...
    'Open Field Profiles at 3cm.csv',  ...
...
figure('Name','PDD – 3x3 cm², TrueBeam 15X','Position',[50 100 700 500]);
...
title('PDD – TrueBeam 15X, 3×3 cm², SSD=100 cm');
...
for k = 1:length(profile_depths_cm)
    figure('Name', sprintf('Profile %.0fcm – 3x3 cm²', profile_depths_cm(k)), ...
    ...
    ylabel('Relative Dose [%]');
    title(sprintf('Profile at %.0f cm (grid: %.1f mm) – TrueBeam 15X, 3×3 cm²', ...
...
x_check = [1, 1.5, 2, 3];
for k = 1:length(profile_depths_cm)
    fprintf('  Depth = %.0f cm (grid %.1f mm)\n', ...
        profile_depths_cm(k), depth_used_mm(k));
    for x_off = x_check
        for sgn = [1, -1]
            xq = sgn * x_off;
...
%% 15. Save
save('s7_water_phantom_3x3_15x_results.mat', ...
    ...
    'profile_depths_cm');
fprintf('\nSaved: s7_water_phantom_3x3_15x_results.mat\n');
