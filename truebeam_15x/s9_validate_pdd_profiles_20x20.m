%% s7_validate_pdd_profiles_20x20.m  –  TrueBeam 15X (15MV flat)
...
figure('Name','PDD – 20x20 cm², TrueBeam 15X','Position',[50 100 700 500]);
...
title('PDD – TrueBeam 15X, 20×20 cm², SSD=100 cm');
...
for k = 1:length(profile_depths_cm)
    ...
    ylabel('Relative Dose [%]');
    title(sprintf('Profile at %.0f cm (grid: %.1f mm) – TrueBeam 15X, 20×20 cm²', ...
...
%% 15. Save
save('s9_water_phantom_20x20_15x_results.mat', ...
    ...
    'profile_depths_cm');
fprintf('\nSaved: s9_water_phantom_20x20_15x_results.mat\n');
