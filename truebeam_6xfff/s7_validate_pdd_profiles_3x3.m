%% s9_validate_pdd_profiles_3x3.m
...
%   Ray count  : ~169 bixels (13x13 at 2.5mm spacing)
%
% addMargin=false: prevents matRad from auto-dilating the target by
%   bixelWidth (2.5mm), which would shift rays from ±15mm to ±17.5mm
...
[depth_meas_cm, pdd_meas_pct] = readGBDColumn( ...
    fullfile(gbdDir, 'Open Field Depth Dose.csv'), 6, 7, '3x3cm2');
...
for k = 1:length(profile_depths_cm)
    [xm, dm] = readGBDColumn(fullfile(gbdDir, profile_files{k}), 8, 9, '3x3 cm2');
...
figure('Name','PDD – 3x3 cm², TrueBeam 6FFF','Position',[50 100 700 500]);
...
title('PDD – TrueBeam 6XFFF, 3×3 cm², SSD=100 cm');
...
for k = 1:length(profile_depths_cm)
    ...
    ylabel('Relative Dose [%]');
    title(sprintf('Profile at %.1f cm (grid: %.1f mm) – TrueBeam 6XFFF, 3×3 cm²', ...
...
%% 15. Save
save('s9_water_phantom_3x3_results.mat', ...
    ...
    'profile_depths_cm');
fprintf('\nSaved: s9_water_phantom_3x3_results.mat\n');
