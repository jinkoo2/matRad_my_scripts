%% s8_validate_pdd_profiles_10x10.m
% Validate TrueBeam 6XFFF machine in a homogeneous water phantom.
% Setup  : AP beam (gantry 0), 10x10 cm² open field, SSD = 100 cm.
% Outputs: (1) calculated PDD vs GBD 10x10 cm² PDD
%          (2) calculated crossline profiles at 1.5, 5, 10, 20, 30 cm
%              vs GBD 10x10 cm² profiles
%
% Phantom is sized for 10x10 cm² to minimize runtime:
%   x/z : ±130 mm (covers GBD profile range of ±114 mm at 30 cm depth)
%   y   : 0–318 mm (covers GBD PDD range of 0–30 cm)
%   Voxel count: 130×160×130 = 2.7 M  vs  14.4 M for 40x40 script (~5× faster)
%   Ray count  : ~484 bixels           vs  ~6400 (~13× fewer)
%
% Note: 2 mm resolution means the closest grid depth to 1.5 cm is
%       14 mm (1.4 cm) or 16 mm (1.6 cm); min-distance selection is used.
...
Nx = 130;   % cols   x: -130 to +128 mm  (inline,     ±130 mm)
Ny = 160;   % rows   y:    0 to +318 mm  (depth, beam direction)
Nz = 130;   % slices z: -130 to +128 mm  (crossline,  ±130 mm)

ct.x = (-(Nx/2) : (Nx/2 - 1)) * res;   % -130, -128, ..., 128 mm
ct.y = (0       : Ny - 1)      * res;   %    0,    2, ..., 318 mm
ct.z = (-(Nz/2) : (Nz/2 - 1)) * res;   % -130, -128, ..., 128 mm
...
% Target: exactly the 10x10 cm² field footprint (±50 mm).
% The STF generator projects these voxels to place pencil-beam rays.
% Do NOT add extra margin here — it would place rays outside the field
% boundary and produce a larger-than-intended field size.
ix_tgt = find(ct.x >= -50 & ct.x <= 50);
iz_tgt = find(ct.z >= -50 & ct.z <= 50);
...
cst{2,1} = 1;  cst{2,2} = 'PTV_10x10'; cst{2,3} = 'TARGET';
cst{2,4} = {V_target}; cst{2,5} = meta_ptv;  cst{2,6} = {};

%% 4. Plan: single AP beam, 10x10 cm², gantry 0°
...
% Disable the automatic target margin that matRad adds before projecting
% rays (addMargin=true by default, expands target by bixelWidth=5mm in
% every direction).  For a 40x40 field that margin is negligible (~1%),
% but for a 10x10 field it shifts the outermost rays from ±50mm to ±55mm,
% producing an ~11x11 cm² field.  Setting addMargin=false ensures rays
% are placed exactly at the specified field boundary (±50mm).
...
gbdDir = '/gpfs/projects/KimGroup/projects/tps/matRad/my_scripts/TrueBeamGBD/6FFF Beam Data';

% Helper: read a 2-column (position, value) dataset from a GBD CSV.
%   hdr_row : 1-based row index of column-header row
%   dat_row : 1-based row index where data starts
%   tag     : substring to match the desired field-size column
...
% -- PDD --
% CSV structure: rows 1-5 metadata, row 6 headers, rows 7+ data
[depth_meas_cm, pdd_meas_pct] = readGBDColumn( ...
    fullfile(gbdDir, 'Open Field Depth Dose.csv'), 6, 7, '10x10');
depth_meas_mm = depth_meas_cm * 10;
pdd_meas_norm = 100 * pdd_meas_pct / max(pdd_meas_pct);

% -- Profiles --
% CSV structure: rows 1-7 metadata, row 8 headers, rows 9+ data
profile_files = { ...
...
for k = 1:length(profile_depths_cm)
    [xm, dm] = readGBDColumn(fullfile(gbdDir, profile_files{k}), 8, 9, '10x10');
...
figure('Name','PDD – 10x10 cm², TrueBeam 6FFF','Position',[50 100 700 500]);
...
title('PDD – TrueBeam 6XFFF, 10×10 cm², SSD=100 cm');
...
for k = 1:length(profile_depths_cm)
    figure('Name', sprintf('Profile %.0fcm – 10x10 cm²', profile_depths_cm(k)), ...
    ...
    xline([-5 5], 'k:', 'LineWidth',0.8);    % field edge markers
    xlabel('Off-axis position [cm]');
    ylabel('Relative Dose [%]');
    title(sprintf('Profile at %.1f cm (grid: %.1f mm) – TrueBeam 6XFFF, 10×10 cm²', ...
...
fprintf('\n=== Profile summary – 10×10 cm² ===\n');
x_check = [1, 2, 4, 5, 6];   % off-axis positions to report [cm]
for k = 1:length(profile_depths_cm)
...
%% 15. Save
save('s8_water_phantom_10x10_results.mat', ...
    ...
    'profile_depths_cm');
fprintf('\nSaved: s8_water_phantom_10x10_results.mat\n');
