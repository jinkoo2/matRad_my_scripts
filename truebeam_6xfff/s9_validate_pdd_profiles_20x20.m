%% s7_validate_pdd_profiles_20x20.m
...
%   bixelWidth (10mm), which would shift rays from ±100mm to ±110mm
...
% every direction).  For a 20x20 field this shifts the outermost rays from
% ±100mm to ±105mm, producing a ~21x21 cm² field.
% Setting addMargin=false ensures rays are placed exactly at ±100mm.
...
figure('Name','PDD – 20x20 cm², TrueBeam 6FFF','Position',[50 100 700 500]);
...
title('PDD – TrueBeam 6XFFF, 20×20 cm², SSD=100 cm');
...
for k = 1:length(profile_depths_cm)
    ...
    ylabel('Relative Dose [%]');
    title(sprintf('Profile at %.1f cm (grid: %.1f mm) – TrueBeam 6XFFF, 20×20 cm²', ...
...
x_check = [2, 5, 8, 10, 12];   % off-axis positions to report [cm]
for k = 1:length(profile_depths_cm)
...
%% 15. Save
save('s7_water_phantom_20x20_results.mat', ...
    ...
    'profile_depths_cm');
fprintf('\nSaved: s7_water_phantom_20x20_results.mat\n');
